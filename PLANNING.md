...
- **Enter-key conflict (resolved 2026-08-21):** `Enter` is now the
  main action verb on every row — it no longer toggles branches.
  Expand/collapse is `→`/`←`/`Space`/`Tab` only. On a branch,
  `Enter` steps into it (toward its first child); on a leaf it runs
  the default action. Spec §8 amended (keymap table + acceptance
  criterion #7) to match.
